...
/****************SUBSCRIPTION PLANS*********************/
function updateSubcriptionValue() {
  const selector = $('.subscription-plans-dropdown').find(":selected");
  const priceAdjustmentType = selector.data('price-adjustment-type');
  const subscribeFixedValue = selector.data('saved-on-subscription');
  const subscribeDiscountedValue = selector.data('saved-percentage-on-subscription');

  $('.subscribe-offer-text').css('display', subscribeDiscountedValue > 0 ? '' : 'none');
  if( priceAdjustmentType === "fixed_amount"){
    $('.subscribe-discount-value').text(subscribeFixedValue);
  } else {
    $('.subscribe-discount-value').text(subscribeDiscountedValue + '%');
  }
}
