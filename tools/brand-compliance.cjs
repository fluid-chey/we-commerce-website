 ...
 * against brand rules defined as static constants (no database required).
 *
 ...
 *
 * Ported from Fluid-DesignOS — all DB dependencies removed.
...
// Brand rule constants — authoritative values derived from Fluid brand docs.
const BASE_RULES = {
  version: '2.0.0',
  colors: {
  ...
  fonts: {
    ...
    website_families: ['NeueHaasDisplay', 'NeueHaas', 'FLFont', 'flfontbold', 'Inter'],
...
function checkMultilingualAccents(lines, rules, context) {
  ...
  if (uppercaseBlocks.test(joined)) {
    // Advisory: uppercase transform preserves accents in modern browsers
  }
  for (let i = 0; i < lines.length; i++) {
...
Brand rules are loaded from built-in constants (no database required).
`);
  process.exit(0);
}
